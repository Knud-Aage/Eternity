...
namespace EternityII_Solver
{
    public static class Util
    {
        ...
        // 2026-09-02 experiment, extended 2026-09-04 to all four non-center hints: grants each
        // hint's candidate table (built with allowBreaks=true, see Program.cs) exactly one break
        // of budget, so if the (west,south) colours the search happens to produce at its cell
        // don't exactly match what the pinned piece needs, the branch can fall back to a one-side
        // match instead of dead-ending with zero fallback.
        //
        // CORRECTION 2026-09-04, then extension same day: the original {34, 45} was believed
        // (wrongly) to be hint208/hint255's own fill-steps. Verified against the live
        // master_piece_lookup that 34 and 45 are actually hint181's and hint249's own steps,
        // while hint208 sits at 188 and hint255 at 247 -- and those two were hard, zero-tolerance
        // pins the whole time this list existed, very likely the real cause of the population
        // bottlenecking hard at step 34 (the first of the four in fill order, with no fallback at
        // all). Rather than just fix the comment, extended the mechanism itself: hint181 and
        // hint249 now also get allowBreaks=true, with 188 and 247 added here so each of the four
        // hints unlocks its OWN +1 of budget exactly at its own step.
        //
        // This isn't cosmetic -- Get_Break_Array() is cumulative and GLOBAL (see the
        // breaks_this_turn check in the search loop: break_array[solve_index] -
        // cumulative_breaks[solve_index - 1]), so every break-tolerant hint draws from the SAME
        // pool, in fill order. With only {34, 45} unlocked, giving hint181/hint249
        // break-tolerance too would let them exhaust that 2-slot pool before hint208 (188) and
        // hint255 (247) ever get a turn, silently starving the two hints this mechanism was
        // originally built for. Adding 188 and 247 as their own unlock points guarantees each
        // hint has its own +1 available at its own step regardless of what any earlier hint
        // spent -- worst case (all four need a break) still resolves, since the pool has grown to
        // 4 by the time the last hint (255) is reached. See BwUtil.HINT_BREAK_INDEXES in the Java
        // ports for the same fix.
        //
        ...
        // middles_no_break vs middles_with_break. Folding these in would have dropped that
        ...
        private static readonly List<int> hint_break_indexes = new List<int>() { 34, 45, 188, 247 };
        ...
        public static void Save_Board(RotatedPiece[] board, ushort maxSolveIndex)
        {
            ...
            string link = "https://e2.bucas.name/#puzzle=Joshua_Blackwood&board_w=16&board_h=16&board_edges=" + url + "&motifs_order=jblackwood";
            ...
            // NOT MyDocuments. On this machine Documents is OneDrive-redirected
            // by Known Folder Move, so every board written here was uploaded to
            // the cloud: 389,856 files / ~1 GB by 2026-08-13, which filled the
            // quota and made OneDrive prompt repeatedly about mass deletions.
            // Board files are bulk intermediate output -- there is no reason to
            // sync them anywhere.
            //
            // UserProfile is never touched by Known Folder Move, so this stays
            // local. Override with ETERNITY_SOLUTIONS_DIR if the boards should
            // live elsewhere (a different drive, for instance).
            ...
            // 2026-08-16: every one of the 14,850 files this folder held before
            // today was labelled ONLY by depth, never by conflict count -- nobody
            // had actually looked at what any of them completed to, which is
            // exactly how a genuinely good result could sit here unnoticed. Label
            // going forward instead of letting that pile back up: reuse the
            // existing, already-tested dk.puzzle.tools.HoleSolver CLI (same tool
            // used all day for this) rather than reimplementing hole-filling here.
            // Wrapped so ANY failure here (java missing, timeout, bad classpath)
            // can never affect the actual search -- this only runs after the raw
            // save has already succeeded.
            TryLabelWithConflictCount(link, maxSolveIndex, path, Path.Combine(path, filename));
        }

        private static void TryLabelWithConflictCount(string link, ushort maxSolveIndex, string saveDir, string baseboardFilePath)
        {
            const string eternityRoot = @"C:\Users\knuda\IdeaProjects\Eternity";
            try
            {
                // HoleSolver loads pieces.csv via a bare relative FileReader("pieces.csv")
                // -- confirmed 2026-08-16 by reading Eternity.loadPieces() directly, after
                // an earlier version of this method set WorkingDirectory to saveDir and
                // it silently fell back to "mock data" (logged at INFO, not thrown) rather
                // than failing loudly. So this MUST run from eternityRoot for real piece
                // data, and its own ErrorsNN_... output (also written relative to CWD)
                // has to be moved into saveDir afterward instead of landing there directly.
                ...
                psi.ArgumentList.Add("dk.puzzle.tools.HoleSolver");
                psi.ArgumentList.Add(link);
                psi.ArgumentList.Add("5000");
                psi.ArgumentList.Add(maxSolveIndex.ToString()); // baseLabel -> "BaseNNN" in the output filename

                var stdout = new StringBuilder();
                using (var proc = Process.Start(psi))
                {
                    // Read async while waiting -- HoleSolver's own verbose per-region
                    // output can fill the OS pipe buffer and deadlock this process
                    // against WaitForExit if nothing drains it, which is the likely
                    // real cause of the timeout warning below firing even on boards
                    // that otherwise complete in seconds.
                ...
                // Move the two files this specific invocation just wrote (matched by the
                // baseLabel we passed in, plus a LastWriteTime after we launched -- narrow
                // enough that unrelated pre-existing Errors*_Base<depth>_* files, if any,
                // are never touched) from the project root into saveDir.
                string pattern = "Errors*_Base" + maxSolveIndex + "_*";
                var newFiles = Directory.GetFiles(eternityRoot, pattern)
                    .Where(f => System.IO.File.GetLastWriteTime(f) >= launchTime.AddSeconds(-1))
                    .ToList();
                foreach (string f in newFiles)
                {
                    string dest = Path.Combine(saveDir, Path.GetFileName(f));
                    System.IO.File.Move(f, dest, overwrite: true);
                }

                // HoleSolver prints the COMPLETED (hole-filled) board's link -- the one
                // actually worth having, unlike the partial-board link this method was
                // handed as a parameter -- after this exact marker. Relay it into
                // whatever this process's own stdout is redirected to, so a good result
                // shows up right in the run log instead of requiring an offline
                // reconstruction pass over the saved grid file later.
                const string marker = "=== Final result (use this link) ===";
                ...
                // Retention: keep only what's within 1 conflict of the best labelled
                // result currently on disk in this folder, delete the rest. Recomputed
                // from what's actually on disk every time (not tracked as running state)
                // so a later improvement retroactively cleans out now-stale near-misses
                // too, not just future saves -- the same policy manually applied to the
        ...
        private static void PruneAboveThreshold(string saveDir)
        {
            try
            {
                var conflictsByFile = new Dictionary<string, int>();
                foreach (string f in Directory.GetFiles(saveDir, "Errors*_RawBoard.txt"))
                {
                    var m = Regex.Match(Path.GetFileName(f), @"^Errors(\d+)_");
                    if (m.Success) conflictsByFile[f] = int.Parse(m.Groups[1].Value);
                }
